package com.kaka.cloud.util;

import com.kaka.cloud.annotation.ExcelField;
import org.apache.poi.ss.usermodel.*;

import java.io.File;
import java.io.FileInputStream;
import java.io.IOException;
import java.io.InputStream;
import java.lang.reflect.Field;
import java.lang.reflect.InvocationTargetException;
import java.lang.reflect.Method;
import java.text.SimpleDateFormat;
import java.util.*;
import java.util.regex.Matcher;
import java.util.regex.Pattern;

public class ImportExcel {
    // 正则表达式 用于匹配属性的第一个字母
    private static final String REGEX = "[a-zA-Z]";
    // 是否打印提示信息
    private static boolean showInfo = false;

    /**
     * 根据文件加载
     * 
     * @param file
     * @param startRow
     * @param endRow
     * @param clazz
     * @return
     * @throws IOException
     */
    public static List<?> importExcelForFile(File file, int startRow, int endRow, Class<?> clazz) throws IOException {
        if (file == null || !file.exists()) {
            throw new IOException("Excel文件不存在！");
        }
        return doImportExcel(new FileInputStream(file), startRow, endRow, clazz);
    }

    /**
     * 根据文件路径加载
     * 
     * @param originUrl
     * @param startRow
     * @param endRow
     * @param clazz
     * @return
     * @throws IOException
     */
    public static List<?> importExcelForFile(String originUrl, int startRow, int endRow, Class<?> clazz)
            throws IOException {
        // 判断文件是否存在
        File file = new File(originUrl);
        if (!file.exists()) {
            throw new IOException("文件名为" + file.getName() + "Excel文件不存在！");
        }
        return importExcelForFile(file, startRow, endRow, clazz);
    }

    /**
     * 根据流加载
     * 
     * @param inp
     * @param startRow
     * @param endRow
     * @param clazz
     * @return
     * @throws IOException
     */
    public static List<?> importExcelForIo(InputStream inp, int startRow, int endRow, Class<?> clazz)
            throws IOException {
        if (inp == null) {
            throw new IOException("Excel流不存在！");
        }
        return doImportExcel(inp, startRow, endRow, clazz);
    }

    /**
     * 功能:真正实现导入
     */
    private static List<?> doImportExcel(InputStream inp, int startRow, int endRow, Class<?> clazz) throws IOException {
        Workbook wb = null;
        List<Object> objectList = new ArrayList<Object>();
        Object obj = null;
        String attribute = null;
        String value = null;
        try {
            Field[] declaredFields = clazz.getDeclaredFields();
            // 去读Excel
            wb = WorkbookFactory.create(inp);
            Sheet sheet = wb.getSheetAt(0);
            // 获取最后行号
            int lastRowNum = sheet.getLastRowNum();
            if (lastRowNum > 0) { // 如果>0，表示有数据
                out("\n开始读取名为【" + sheet.getSheetName() + "】的内容：");
            } else {
                return objectList;
            }

            /* 加载头信息 */
            Map<String, Integer> fieldIndexMap = new HashMap<String, Integer>();
            Row titleRow = sheet.getRow(startRow - 1);
            for (int cellIndex = 0; cellIndex < titleRow.getLastCellNum(); cellIndex++) {
                fieldIndexMap.put(getCellValue(titleRow.getCell(cellIndex)), cellIndex);
            }
            for (Field field : declaredFields) {
                attribute = field.getName().toString();
                ExcelField excelField = field.getAnnotation(ExcelField.class);
                if (excelField != null) {
                    Integer index = fieldIndexMap.get(excelField.name());
                    if (index == null) {
                        throw new Exception("模板错误！");
                    }
                }
            }
            // 循环读取
            for (int i = startRow; i <= lastRowNum + endRow; i++) {
                Row row = sheet.getRow(i);
                if (row == null) {
                    break;
                }
                obj = clazz.newInstance();
                for (Field field : declaredFields) {
                    attribute = field.getName().toString();
                    ExcelField excelField = field.getAnnotation(ExcelField.class);
                    if (excelField != null) {
                        Integer index = fieldIndexMap.get(excelField.name());
                        if (index == null) {
                            throw new Exception("模板错误：【" + excelField.name() + "】列，不存在！");
                        }
                        int maxLength = excelField.maxLength();
                        value = getCellValue(row.getCell(index));
                        if (excelField.required() && (value == null || "".equals(value.trim()))) {
                            throw new Exception("第" + (i + 1) + "行：【" + excelField.name() + "】必填！");
                        }
                        value = value.trim();
                        if (maxLength < value.length()) {
                            throw new Exception("第" + (i + 1) + "行：【" + excelField.name() + "】长度过长，实际长度："
                                    + value.length() + "，最大长为：" + maxLength + "; " + value);
                        }
                        setAttrributeValue(obj, attribute, value);
                    }
                }
                objectList.add(obj);

            }
        } catch (Exception e) {
            throw new RuntimeException(e.getMessage());
        } finally {
            try {
                if (inp != null) {
                    inp.close();
                }
            } catch (Exception e) {
            }
        }
        return objectList;
    }

    /**
     * 功能:获取单元格的值
     */
    private static String getCellValue(Cell cell) {
        Object result = "";
        if (cell != null) {
            cell.setCellType(Cell.CELL_TYPE_STRING);
            result = cell.getStringCellValue();
            // TODO 需根据类型判断返回值
            /*
             * switch (cell.getCellType()) { case Cell.CELL_TYPE_STRING: result
             * = cell.getStringCellValue(); break; case Cell.CELL_TYPE_NUMERIC:
             * result = cell.getNumericCellValue(); break; case
             * Cell.CELL_TYPE_BOOLEAN: result = cell.getBooleanCellValue();
             * break; case Cell.CELL_TYPE_FORMULA: result =
             * cell.getCellFormula(); break; case Cell.CELL_TYPE_ERROR: result =
             * cell.getErrorCellValue(); break; case Cell.CELL_TYPE_BLANK:
             * break; default: break; }
             */
        }
        return result.toString();
    }

    /**
     * 功能:给指定对象的指定属性赋值
     */
    private static void setAttrributeValue(Object obj, String attribute, String value) {
        // 得到该属性的set方法名
        String methodName = convertToMethodName(attribute, obj.getClass(), true);
        Method[] methods = obj.getClass().getMethods();
        for (Method method : methods) {
            /**
             * 因为这里只是调用bean中属性的set方法，属性名称不能重复 所以set方法也不会重复，所以就直接用方法名称去锁定一个方法
             * （注：在java中，锁定一个方法的条件是方法名及参数）
             */
            if (method.getName().equals(methodName)) {
                Class<?>[] parameterC = method.getParameterTypes();
                try {
                    /**
                     * 如果是(整型,浮点型,布尔型,字节型,时间类型), 按照各自的规则把value值转换成各自的类型
                     * 否则一律按类型强制转换(比如:String类型)
                     */
                    if (parameterC[0] == int.class || parameterC[0] == Integer.class) {
                        value = value.substring(0, value.lastIndexOf("."));
                        method.invoke(obj, Integer.valueOf(value));
                        break;
                    } else if (parameterC[0] == float.class || parameterC[0] == Float.class) {
                        method.invoke(obj, Float.valueOf(value));
                        break;
                    } else if (parameterC[0] == double.class || parameterC[0] == Double.class) {
                        method.invoke(obj, Double.valueOf(value));
                        break;
                    } else if (parameterC[0] == byte.class || parameterC[0] == Byte.class) {
                        method.invoke(obj, Byte.valueOf(value));
                        break;
                    } else if (parameterC[0] == boolean.class || parameterC[0] == Boolean.class) {
                        method.invoke(obj, Boolean.valueOf(value));
                        break;
                    } else if (parameterC[0] == Date.class) {
                        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
                        Date date = null;
                        try {
                            date = sdf.parse(value);
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                        method.invoke(obj, date);
                        break;
                    } else {
                        method.invoke(obj, parameterC[0].cast(value.replace(" ", "")));
                        break;
                    }
                } catch (IllegalArgumentException e) {
                    e.printStackTrace();
                } catch (IllegalAccessException e) {
                    e.printStackTrace();
                } catch (InvocationTargetException e) {
                    e.printStackTrace();
                } catch (SecurityException e) {
                    e.printStackTrace();
                }
            }
        }
    }

    /**
     * 功能:根据属性生成对应的set/get方法
     */
    private static String convertToMethodName(String attribute, Class<?> objClass, boolean isSet) {
        /** 通过正则表达式来匹配第一个字符 **/
        Pattern p = Pattern.compile(REGEX);
        Matcher m = p.matcher(attribute);
        StringBuilder sb = new StringBuilder();
        /** 如果是set方法名称 **/
        if (isSet) {
            sb.append("set");
        } else {
            /** get方法名称 **/
            try {
                Field attributeField = objClass.getDeclaredField(attribute);
                /** 如果类型为boolean **/
                if (attributeField.getType() == boolean.class || attributeField.getType() == Boolean.class) {
                    sb.append("is");
                } else {
                    sb.append("get");
                }
            } catch (SecurityException e) {
                e.printStackTrace();
            } catch (NoSuchFieldException e) {
                e.printStackTrace();
            }
        }
        /** 针对以下划线开头的属性 **/
        if (attribute.charAt(0) != '_' && m.find()) {
            sb.append(m.replaceFirst(m.group().toUpperCase()));
        } else {
            sb.append(attribute);
        }
        return sb.toString();
    }

    /**
     * 功能:输出提示信息(普通信息打印)
     */
    private static void out(String info) {
        if (showInfo) {
            System.out.print(info + (showInfo ? "\n" : ""));
        }
    }


}
